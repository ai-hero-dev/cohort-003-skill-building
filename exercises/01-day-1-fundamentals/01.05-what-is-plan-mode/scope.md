...
## Scope
Introduce plan mode as a foundational technique that solves the core problem of AI code quality — by forcing the agent to explore and understand a codebase thoroughly *before* writing any code, you get better results and clearer requirements.
...
## In Scope
- **What plan mode is**: The specific restrictions it places (no file editing, no command execution, no test running) and what it *allows* (reading, exploring, analyzing)
...
- **The psychological benefit to the developer**: How iterating on requirements with the agent *before* coding forces clarity on what you actually want (decision-tree metaphor)
...
- **Context window priming**: Why *not* clearing context between planning and execution leads to better code (the agent keeps loaded context about patterns and decisions)
...
2. **Define plan mode concretely** — Show the permission table. What's blocked? (File editing, command execution, tests.) What's allowed? (Reading, exploring, analyzing.) Make it clear this is a *constraint that forces focus*.
